! function() {
    ...
    function e(t) {
        var n = .5 * t[0] * l,
            ...
            s = Math.sin(a),
            h = Math.cos(a);
        return [r * c * h + o * i * s, o * c * h - r * i * s, r * i * h + o * c * s, r * c * s - o * i * h]
    }
    ...
    function a(t, n) {
        var e = t[0],
            ...
            s = n[2],
            h = n[3];
        return [e * i - a * c - o * s - r * h, e * c + a * i + o * h - r * s, e * s - a * h + o * i + r * c, e * h + a * s - o * c + r * i]
    }
    ...
    d3.geo.zoom = function() {
        ...
        var f, d, M, m = 0,
            v = u(g, "zoomstart", "zoom", "zoomend"),
            g = d3.behavior.zoom().on("zoomstart", function() {
                var r = d3.mouse(this),
                    c = e(f.rotate()),
                    u = n(f, r);
                u && (M = u), p.call(g, "zoom", function() {
                    f.scale(z.k = d3.event.scale);
                    var e = d3.mouse(this),
                        s = o(M, n(f, e));
                    f.rotate(z.r = i(c = s ? a(c, s) : a(t(f, r, e), c))), r = e, h(v.of(this, arguments))
                }), s(v.of(this, arguments))
            }).on("zoomend", function() {
                p.call(g, "zoom", null), l(v.of(this, arguments))
            }),
            p = g.on,
            z = {
                r: [0, 0, 0],
            ...
            t.each(function() {
                ...
                if (o !== t) {
                    o.each("start.zoom", function() {
                        ...
                        return d && o.duration(d(.001 * u.duration)),
                            function(e) {
                                var a = u(e);
                                this.__chart__ = z = {
                                    r: i(c(a[0] / s)),
                                    k: t / a[2]
                                }, f.rotate(z.r).scale(z.k), g.scale(z.k), h(n)
                            }
...
function() {
    function t(t) {
        return {
            moveTo: function(n, a) {
                t.moveTo(Math.round(n * e), Math.round(a * e))
            },
            lineTo: function(n, a) {
                t.lineTo(Math.round(n * e), Math.round(a * e))
            },
            closePath: function() {
                t.closePath()
            }
    ...
    var n = 180 / Math.PI,
        ...
        a = $("#banner-content").width(),
        o = 850,
        r = e,
        i = d3.geo.orthographic().rotate([0, -30]).scale(o / 2 - 1).translate([a / 2, o / 2]).clipExtent([
            [-r, -r],
            [a + r, o + r]
        ]).precision(.5),
        c = d3.select("#map").append("canvas").attr("width", a * e).attr("height", o * e).style("width", a + "px").style("height", o + "px"),
        s = c.node().getContext("2d"),
        h = d3.geo.path().projection(i).context(t(s)),
        u = d3.select("#north-up").on("change", function() {
            u = this.checked
        }).property("checked");
    d3.json("./world-110m.json", function(t, r) {
        function l() {
            f(i, g[p = ((z = p) + 1) % g.length]), c.transition().ease("quad-in-out").duration(2e3).call(b.projection(i).event)
        }

        function f(t, n) {
            var e = d3.geo.centroid(n),
                r = t.clipExtent();
            t.rotate(u ? [-e[0], -e[1]] : b.rotateTo(e)).clipExtent(null).scale(1).translate([0, 0]);
            var i = h.bounds(n),
                c = Math.min(1e3, .45 / Math.max(Math.max(Math.abs(i[1][0]), Math.abs(i[0][0])) / a, Math.max(Math.abs(i[1][1]), Math.abs(i[0][1])) / o));
            t.clipExtent(r).scale(c).translate([a / 2, o / 2])
        }
        var d = {
                type: "Sphere"
            },
            ...
            g = d3.shuffle(topojson.feature(r, r.objects.countries).features),
            p = -1,
            z = p,
            b = d3.geo.zoom().projection(i).duration(function(t) {
                return 2e3 * Math.sqrt(t)
            }).scaleExtent([o / 2 - 1, 1 / 0]).on("zoom", function() {
                i.clipAngle(Math.asin(Math.min(1, 5 * Math.sqrt(a * a + o * o) / i.scale())) * n), s.clearRect(0, 0, a * e, o * e), s.strokeStyle = "#000000", s.lineWidth = .5 * e, s.beginPath(), h(M), s.stroke(), s.fillStyle = "rgba(0, 0, 0, 0.1)", s.beginPath(), h(m), s.fill(), s.fillStyle = "#rgba(255, 0, 0, 0.4)", s.beginPath(), h(g[z]), s.fill(), s.fillStyle = "#rgba(0, 255, 0, 0.4)", s.beginPath(), h(g[p]), s.fill(), s.strokeStyle = "#000000", s.lineWidth = .5 * e, s.beginPath(), h(v), s.stroke(), s.strokeStyle = "#000", s.lineWidth = 2 * e, s.beginPath(), h(d), s.stroke()
            }).on("zoomend", l);
        c.call(b).call(b.event)
    })
}();